package com.onetry.spring.volunteering.controller;
...
import com.onetry.spring.volunteering.service.VolunteeringService;
...
@Tag(name = "[Volunteering] Volunteering API 봉사활동 을 저장, 조회, 삭제 합니다.")
@RestController
...
public class VolunteeringController {
    ...
    @GetMapping("/all")
    public ResponseEntity<CommonResponseDto<List<VolunteeringInfo>>> getAllVolunteering(@AuthenticationPrincipal UserDetails userDetails){
    ...
    public ResponseEntity<CommonResponseDto<VolunteeringInfo>> addTranscript(@ModelAttribute VolunteeringCreateDto volunteeringCreateDto,
    ...
    public ResponseEntity<CommonResponseDto<Void>> deleteVolunteering(@PathVariable("volunteeringId") Long volunteeringId,
                                                                       @AuthenticationPrincipal UserDetails userDetails){
        String email = userDetails.getUsername();
